...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("🏆 *BENVENUTO!* Preparati...", parse_mode="Markdown")
...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("adm_") and username in ADMIN_USERS:
        if data == "adm_view":
            ...
            await query.message.reply_text(txt if top else "Vuoto.", parse_mode="Markdown")
        elif data == "adm_drop_db":
            players.delete_many({})
            await query.edit_message_text("✅ Reset completato.")
        await query.answer()
        return

    if p.get("game_over") and username not in ADMIN_USERS:
        await query.answer("Fine gioco.")
        return

    # RISPOSTE
    if data.startswith("ans_"):
        scelta = data.split("_")[1] # Prende 'A', 'B', etc.
        await pulisci_messaggi_aiuto(user_id, context)
        ...
        if scelta == q["c"]:
            ...
            players.update_one({"user_id": user_id}, {"$set": {"game_over": True}})
            await query.edit_message_text(f"❌ *SBAGLIATO!* Era {q['c']}.", parse_mode="Markdown")
    ...
    # AIUTI
        ...
        if tipo == "5050":
            ...
            txt = f"📊 *Pubblico*: {q['c']}" if tipo == "pub" else f"📞 *Tel*: '{q['c']}'"
...
async def admin_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if update.effective_user.username in ADMIN_USERS:
        kb = InlineKeyboardMarkup([[InlineKeyboardButton("Classifica", callback_data="adm_view"), InlineKeyboardButton("Reset", callback_data="adm_drop_db")]])
